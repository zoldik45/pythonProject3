...
def main():
    ...
    for i in range(1, 9):
        image = pygame.image.load(f'images/flag_{i}.png').convert_alpha()
        # Считываем изображений кадров
    ...
    def show_victory_message():
        ...
        scoreitog = score * 100 / (elapsed_seconds / 4)
        poland = int(scoreitog)
        text = font.render(f'Вы выиграли! Время:'
                           f' {elapsed_seconds} сек. Собрано монет:'
                           f' {score}', True, pygame.Color('orange'))
        score_text = font.render(f'Ваш счёт: {poland}',
                                 True, pygame.Color('orange'))
        text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
        score_text_rect = score_text.get_rect(center=(WIDTH // 2,
                                                      text_rect.bottom + 40))

        high_score_text = font.render(f'Лучший счёт: {high_score}',
                                      True, pygame.Color('orange'))
        high_score_rect = (
            high_score_text.get_rect(center=(WIDTH // 2,
                                             score_text_rect.bottom + 40)))

        cursor.execute('INSERT INTO eda (score) VALUES (?)', (scoreitog,))
        conn.commit()

        back_button = Button(200, 50, 'skyblue',
                             'lightskyblue',
                             'Вернуться в меню', font_size=30)
        back_button_rect = back_button.draw(WIDTH // 2 - 100,
                                            high_score_rect.bottom + 60)
    ...
    def show_lose_message():
        ...
        text = font.render(f'Вы проиграли! Время:'
                           f' {elapsed_seconds} сек. Собрано монет:'
                           f' {score}', True,
                           pygame.Color('orange'))
        text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
    ...
    class Player:
        ...
        def draw(self):
            ...
            sc.blit(self.images[self.direction],
    ...
    class Coin:
        def __init__(self, pics, x, y):
            ...
            self.rect = pygame.Rect(x * TILE + TILE // 4,
                                    y * TILE + TILE // 4,
                                    TILE // 2, TILE // 2)
    ...
    coin_images_2 = ["moneta_1.png", "moneta_2.png",
                     "moneta_3.png", "moneta_4.png",
                     "moneta_5.png", "moneta_6.png",
                     "moneta_7.png", "moneta_8.png"]
    coins = [Coin(coin_images_2, randint(1, cols - 2),
                  randint(1, rows - 2)) for _ in range(40)]
    ...
    while True:
        ...
        if enemy is not None:
            current_time = pygame.time.get_ticks()
            if current_time - enemy_move_timer > 1000:
                enemy_move_timer = pygame.time.get_ticks()
        ...
        if animation_time >= frame_duration:
            current_frame =\
                (current_frame + 1) % len(coin_images)  # Переключение кадров
            animation_time = 0

        screen.blit(coin_images[current_frame], (870, 660))
        pygame.display.flip()
        clock.tick(3000)
