...
def main():
    ...
    def show_victory_message():
        ...
        scoreitog = score * 300 / (elapsed_seconds / 4)
        poland = int(scoreitog)
        text = font.render(
            f"Вы выиграли! Время: {elapsed_seconds} сек. Собрано монет:"
            f" {score}",
            True,
            pygame.Color("orange"),
        )
    ...
    def show_lose_message():
        ...
        text = font.render(
            f"Вы проиграли! Время: {elapsed_seconds} сек. Собрано монет:"
            f" {score}",
            True,
            pygame.Color("orange"),
        )
    ...
    while True:
        ...
        if enemy is not None:
            current_time = pygame.time.get_ticks()
            if current_time - enemy_move_timer > 800:
                enemy_move_timer = pygame.time.get_ticks()
        ...
        screen.blit(coin_images[current_frame], (970, 710))
